#!/usr/bin/env python3
# Copyright (c) 2016-2019 The Bitcoin_Silver Core developers
...
def main():
    parser = argparse.ArgumentParser(description='Bitcoin Silver-core msbuild configuration initialiser.')
        ...
        with open(vcxproj_filename + '.in', 'r', encoding='utf-8') as vcxproj_in_file:
            with open(vcxproj_filename, 'w', encoding='utf-8') as vcxproj_file:
                vcxproj_file.write(vcxproj_in_file.read().replace(
                    '@SOURCE_FILES@\n', content))
    copyfile(os.path.join(SOURCE_DIR,'../build_msvc/bitcoin_silver_config.h'), os.path.join(SOURCE_DIR, 'config/bitcoin_silver-config.h'))
