...
uint256 SendCoins(CWallet& wallet, SendCoinsDialog& sendCoinsDialog, const CTxDestination& address, CAmount amount, bool rbf)
{
    ...
    entry->findChild<Bitcoin_SilverAmountField*>("payAmount")->setValue(amount);
...
//
//     QT_QPA_PLATFORM=xcb     src/qt/test/test_bitcoin_silver-qt  # Linux
//     QT_QPA_PLATFORM=windows src/qt/test/test_bitcoin_silver-qt  # Windows
//     QT_QPA_PLATFORM=cocoa   src/qt/test/test_bitcoin_silver-qt  # macOS
void TestGUI(interfaces::Node& node)
{
...

    {
        ...
        QString balanceComparison = Bitcoin_SilverUnits::formatWithUnit(unit, balance, false, Bitcoin_SilverUnits::SeparatorStyle::ALWAYS);
    ...
    // Amount input
    Bitcoin_SilverAmountField* amountInput = receiveCoinsDialog.findChild<Bitcoin_SilverAmountField*>("reqAmount");
    ...
    for (QWidget* widget : QApplication::topLevelWidgets()) {
        if (widget->inherits("ReceiveRequestDialog")) {
            ...
            QCOMPARE(uri.count("bitcoin_silver:"), 2);
...
void WalletTests::walletTests()
{
    ...
    if (QApplication::platformName() == "minimal") {
        ...
        QWARN("Skipping WalletTests on mac build with 'minimal' platform set due to Qt bugs. To run AppTests, invoke "
              "with 'QT_QPA_PLATFORM=cocoa test_bitcoin_silver-qt' on mac, or else use a linux or windows build.");
        return;
    }
#endif
    TestGUI(m_node);
