...
{
    // If Qt is used, parameters/bitcoinsilver.conf are parsed in qt/bitcoinsilver.cpp's main()
...
static bool ProcessInitCommands(ArgsManager& args)
{
    ...
    if (HelpRequested(args) || args.IsArgSet("-version")) {
        ...
        if (args.IsArgSet("-version")) {
            ...
            strUsage += "\nUsage:  bitcoinsilverd [options]                     Start " PACKAGE_NAME "\n"
                "\n";
...
static bool AppInit(NodeContext& node)
{
    ...
    try
    {
        // -server defaults to true for bitcoinsilverd but not for the GUI so do this here
...
MAIN_FUNCTION
{
    ...
    // Connect bitcoinsilverd signal handlers
    noui_connect();
