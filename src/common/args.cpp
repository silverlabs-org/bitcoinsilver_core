...
const char * const BITCOINSILVER_CONF_FILENAME = "bitcoinsilver.conf";
const char * const BITCOINSILVER_SETTINGS_FILENAME = "settings.json";
...
{
    fs::path settings = GetPathArg("-settings", BITCOINSILVER_SETTINGS_FILENAME);
    if (settings.empty()) {
        return false;
    }
    if (backup) {
