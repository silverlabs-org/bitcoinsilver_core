...
static void SetupBitcoin_SilverTxArgs(ArgsManager &argsman)
{
...
{
    SetupBitcoin_SilverTxArgs(gArgs);
    std::string error;
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.IsArgSet("-version")) {
        ...
        std::string strUsage = PACKAGE_NAME " bitcoin_silver-tx utility version " + FormatFullVersion() + "\n";
        if (!gArgs.IsArgSet("-version")) {
            strUsage += "\n"
                "Usage:  bitcoin_silver-tx [options] <hex-tx> [commands]  Update hex-encoded bitcoin_silver transaction\n"
                "or:     bitcoin_silver-tx [options] -create [commands]   Create hex-encoded bitcoin_silver transaction\n"
                "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded bitcoin_silver transaction
