...
// For details see https://github.com/bitcoin_silver/bitcoin_silver/pull/22348.
...
const char * const BITCOIN_SILVER_CONF_FILENAME = "bitcoinsilver.conf";
const char * const BITCOIN_SILVER_SETTINGS_FILENAME = "settings.json";
 ...
 * https://github.com/bitcoin_silver/bitcoin_silver/pull/16097#issuecomment-514627823
 */
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //bitcoin_silver-tx using stdin
        std::string val;
...
bool ArgsManager::GetSettingsPath(fs::path* filepath, bool temp) const
{
    ...
    if (filepath) {
        std::string settings = GetArg("-settings", BITCOIN_SILVER_SETTINGS_FILENAME);
...
#else
    const char* pszModule = "bitcoin_silver";
#endif
    if (pex)
        return strprintf(
...
{
    // Windows: C:\Users\Username\AppData\Roaming\Bitcoin_Silver
    // macOS: ~/Library/Application Support/Bitcoin_Silver
    // Unix-like: ~/.bitcoin_silver
#ifdef WIN32
    // Windows
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", BITCOIN_SILVER_CONF_FILENAME);
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Make sure Bitcoin_Silver Core copyright is not removed by accident
    if (copyright_devs.find("Bitcoin Silver Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The Bitcoin Silver Core developers";
    }
