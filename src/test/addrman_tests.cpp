...
BOOST_AUTO_TEST_CASE(addrman_simple)
{
    ...
    CService addr1 = ResolveService("250.1.1.1", 10566);
    BOOST_CHECK(addrman->Add({CAddress(addr1, NODE_NONE)}, source));
    BOOST_CHECK_EQUAL(addrman->Size(), 1U);
    auto addr_ret1 = addrman->Select().first;
    BOOST_CHECK_EQUAL(addr_ret1.ToStringAddrPort(), "250.1.1.1:10566");

    // Test: Does IP address deduplication work correctly.
    //  Expected dup IP should not be added.
    CService addr1_dup = ResolveService("250.1.1.1", 10566);
    ...
    CService addr2 = ResolveService("250.1.1.2", 10566);
    ...
    vAddr.emplace_back(ResolveService("250.1.1.3", 10566), NODE_NONE);
    vAddr.emplace_back(ResolveService("250.1.1.4", 10566), NODE_NONE);
...
BOOST_AUTO_TEST_CASE(addrman_ports)
{
    ...
    CService addr1_port = ResolveService("250.1.1.1", 10564);
    BOOST_CHECK(addrman->Add({CAddress(addr1_port, NODE_NONE)}, source));
    BOOST_CHECK_EQUAL(addrman->Size(), 2U);
    auto addr_ret2 = addrman->Select().first;
    BOOST_CHECK(addr_ret2.ToStringAddrPort() == "250.1.1.1:10566" || addr_ret2.ToStringAddrPort() == "250.1.1.1:10564");
    ...
    BOOST_CHECK_EQUAL(addr_ret3.ToStringAddrPort(), "250.1.1.1:10566");
}
...
BOOST_AUTO_TEST_CASE(addrman_select)
{
    ...
    CService addr2 = ResolveService("250.3.1.1", 10566);
    ...
    BOOST_CHECK(addrman->Add({CAddress(addr4, NODE_NONE)}, ResolveService("250.4.1.1", 10566)));

    // Add three addresses to tried table.
    CService addr5 = ResolveService("250.4.4.4", 10566);
    CService addr6 = ResolveService("250.4.5.5", 7777);
    CService addr7 = ResolveService("250.4.6.6", 10566);
    ...
    BOOST_CHECK(addrman->Add({CAddress(addr7, NODE_NONE)}, ResolveService("250.1.1.3", 10566)));
...
BOOST_AUTO_TEST_CASE(addrman_select_special)
{
    ...
    // add ipv4 address to the new table
    CService addr1 = ResolveService("250.1.1.3", 10566);
    BOOST_CHECK(addrman->Add({CAddress(addr1, NODE_NONE)}, source));
...
{
    auto addrman = std::make_unique<AddrMan>(EMPTY_NETGROUPMAN, DETERMINISTIC, GetCheckRatio(m_node));
    CAddress addr{CAddress(ResolveService("253.3.3.3", 10566), NODE_NONE)};
...
BOOST_AUTO_TEST_CASE(addrman_getaddr)
{
    ...
    CAddress addr1 = CAddress(ResolveService("250.250.2.1", 10566), NODE_NONE);
    addr1.nTime = Now<NodeSeconds>(); // Set time so isTerrible = false
    CAddress addr2 = CAddress(ResolveService("250.251.2.2", 9999), NODE_NONE);
    addr2.nTime = Now<NodeSeconds>();
    CAddress addr3 = CAddress(ResolveService("251.252.2.3", 10566), NODE_NONE);
    addr3.nTime = Now<NodeSeconds>();
    CAddress addr4 = CAddress(ResolveService("252.253.3.4", 10566), NODE_NONE);
    addr4.nTime = Now<NodeSeconds>();
    CAddress addr5 = CAddress(ResolveService("252.254.4.5", 10566), NODE_NONE);
...
{
    CAddress addr1 = CAddress(ResolveService("250.1.1.1", 10566), NODE_NONE);
...
{
    CAddress addr1 = CAddress(ResolveService("250.1.2.1", 10566), NODE_NONE);
...
BOOST_AUTO_TEST_CASE(load_addrman)
{
    ...
    addr1 = Lookup("250.7.1.1", 10566, false);
    ...
    const std::optional<CService> source{Lookup("252.5.1.1", 10566, false)};
...
BOOST_AUTO_TEST_CASE(addrman_update_address)
{
    ...
    auto addrman = std::make_unique<AddrMan>(EMPTY_NETGROUPMAN, DETERMINISTIC, GetCheckRatio(m_node));
    CNetAddr source{ResolveIP("252.2.2.2")};
    CAddress addr{CAddress(ResolveService("250.1.1.1", 10566), NODE_NONE)};
    ...
    CAddress addr_diff_port{CAddress(ResolveService("250.1.1.1", 10564), NODE_NONE)};
...
BOOST_AUTO_TEST_CASE(addrman_size)
{
    ...
    const CAddress addr1{ResolveService("250.1.1.1", 10566), NODE_NONE};
    BOOST_CHECK(addrman->Add({addr1}, source));
    BOOST_CHECK(addrman->Good(addr1));
    const CAddress addr2{ResolveService("250.1.1.2", 10566), NODE_NONE};
