...
**Bitcoin_Silver Core's `configure` script by default will ignore the depends output.** In
...
RISC-V known issue: gcc-7.3.0 and gcc-7.3.1 result in a broken `test_bitcoin_silver` executable (see https://github.com/bitcoin_silver/bitcoin_silver/pull/13543),
...
options will be passed to bitcoin_silver's configure. In this case, `--disable-wallet`.
