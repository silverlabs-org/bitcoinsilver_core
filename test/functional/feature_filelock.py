...
"""Check that it's not possible to start a second bitcoinsilverd instance using the same datadir or wallet."""
import random
import string
...
class FilelockTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        self.log.info("Check that we can't start a second bitcoinsilverd instance using the same datadir")
        ...
        assert cookie_file.exists()  # should not be deleted during the second bitcoinsilverd instance shutdown
        ...
        if self.is_wallet_compiled():
            def check_wallet_filelock(descriptors):
                ...
                self.log.info("Check that we can't start a second bitcoinsilverd instance using the same wallet")
                if descriptors:
