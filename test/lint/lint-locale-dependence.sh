...
# Be aware that bitcoin_silverd and bitcoin_silver-qt differ in terms of localization: Qt
# opts in to POSIX localization by running setlocale(LC_ALL, "") on startup,
# whereas no such call is made in bitcoin_silverd.
#
...
# C locale.
#
# In contrast, bitcoin_silverd does not opt in to localization -- no call to
# setlocale(LC_ALL, "") is made and the environment variables LC_* are
# thus ignored.
#
# This results in situations where bitcoin_silverd is guaranteed to be running
# with the classic locale ("C") whereas the locale of bitcoin_silver-qt will vary
# depending on the user's environment variables.
#
# An example: Assuming the environment variable LC_ALL=de_DE then the
# call std::to_string(1.23) will return "1.230000" in bitcoin_silverd but
# "1,230000" in bitcoin_silver-qt.
#
...
KNOWN_VIOLATIONS=(
    "src/bitcoin_silver-tx.cpp.*stoul"
    "src/bitcoin_silver-tx.cpp.*trim_right"
