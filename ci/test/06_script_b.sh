...
(
  ...
  # See https://github.com/MrVistos/bitcoinsilver/pull/28359#issuecomment-1698694748
...
if [ "$RUN_FUZZ_TESTS" = "true" ]; then
  ...
  if [ ! -d "$DIR_FUZZ_IN" ]; then
    ${CI_RETRY_EXE} git clone --depth=1 https://github.com/bitcoinsilver-core/qa-assets "${DIR_QA_ASSETS}"
  fi
  (
    cd "${DIR_QA_ASSETS}"
  ...
  if [ ! -d "$DIR_UNIT_TEST_DATA" ]; then
    ...
    ${CI_RETRY_EXE} curl --location --fail https://github.com/bitcoinsilver-core/qa-assets/raw/main/unit_test_data/script_assets_test.json -o "${DIR_UNIT_TEST_DATA}/script_assets_test.json"
  fi
fi
...
if [ "$CI_OS_NAME" == "macos" ]; then
  echo > "${HOME}/Library/Application Support/BitcoinSilver"
else
  echo > "${HOME}/.bitcoinsilver"
fi
...
BITCOINSILVER_CONFIG_ALL="--disable-dependency-tracking"
if [ -z "$NO_DEPENDS" ]; then
  BITCOINSILVER_CONFIG_ALL="${BITCOINSILVER_CONFIG_ALL} CONFIG_SITE=$DEPENDS_DIR/$HOST/share/config.site"
fi
if [ -z "$NO_WERROR" ]; then
  BITCOINSILVER_CONFIG_ALL="${BITCOINSILVER_CONFIG_ALL} --enable-werror"
fi
...
if [ -n "$ANDROID_TOOLS_URL" ]; then
  make distclean || true
  ./autogen.sh
  bash -c "./configure $BITCOINSILVER_CONFIG_ALL $BITCOINSILVER_CONFIG" || ( (cat config.log) && false)
  make "${MAKEJOBS}" && cd src/qt && ANDROID_HOME=${ANDROID_HOME} ANDROID_NDK_HOME=${ANDROID_NDK_HOME} make apk
  bash -c "${PRINT_CCACHE_STATISTICS}"
  exit 0
fi

BITCOINSILVER_CONFIG_ALL="${BITCOINSILVER_CONFIG_ALL} --enable-external-signer --prefix=$BASE_OUTDIR"
...
bash -c "${BASE_ROOT_DIR}/configure --cache-file=config.cache $BITCOINSILVER_CONFIG_ALL $BITCOINSILVER_CONFIG" || ( (cat config.log) && false)

make distdir VERSION="$HOST"

cd "${BASE_BUILD_DIR}/bitcoinsilver-$HOST"

bash -c "./configure --cache-file=../config.cache $BITCOINSILVER_CONFIG_ALL $BITCOINSILVER_CONFIG" || ( (cat config.log) && false)
...
if [ "${RUN_TIDY}" = "true" ]; then
  cmake -B /tidy-build -DLLVM_DIR=/usr/lib/llvm-"${TIDY_LLVM_V}"/cmake -DCMAKE_BUILD_TYPE=Release -S "${BASE_ROOT_DIR}"/contrib/devtools/bitcoinsilver-tidy
  cmake --build /tidy-build "$MAKEJOBS"
  cmake --build /tidy-build --target bitcoinsilver-tidy-tests "$MAKEJOBS"

  set -eo pipefail
  cd "${BASE_BUILD_DIR}/bitcoinsilver-$HOST/src/"
  ( run-clang-tidy-"${TIDY_LLVM_V}" -quiet -load="/tidy-build/libbitcoinsilver-tidy.so" "${MAKEJOBS}" ) | grep -C5 "error"
  ...
  python3 "/include-what-you-use/iwyu_tool.py" \
           -p . "${MAKEJOBS}" \
           -- -Xiwyu --cxx17ns -Xiwyu --mapping_file="${BASE_BUILD_DIR}/bitcoinsilver-$HOST/contrib/devtools/iwyu/bitcoinsilver.core.imp" \
