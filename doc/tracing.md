# User-space, Statically Defined Tracing (USDT) for BitcoinSilver

BitcoinSilver includes statically defined tracepoints to allow for more
...
**Note**: BitcoinSilver uses temporary clones of the _main_ UTXO cache
...
## Adding tracepoints to BitcoinSilver
...
Multiple tools can list the available tracepoints in a `bitcoinsilverd` binary with
USDT support.

### GDB - GNU Project Debugger

To list probes in BitcoinSilver, use `info probes` in `gdb`:

```
$ gdb ./src/bitcoinsilverd
…
(gdb) info probes
Type Provider   Name             Where              Semaphore Object
stap net        inbound_message  0x000000000014419e /src/bitcoinsilverd
stap net        outbound_message 0x0000000000107c05 /src/bitcoinsilverd
stap validation block_connected  0x00000000002fb10c /src/bitcoinsilverd
…
```

### With `readelf`

The `readelf` tool can be used to display the USDT tracepoints in BitcoinSilver.
Look for the notes with the description `NT_STAPSDT`.

```
$ readelf -n ./src/bitcoinsilverd | grep NT_STAPSDT -A 4 -B 2
