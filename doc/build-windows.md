...
Below are some notes on how to build BitcoinSilver for Windows.

The options known to work for building BitcoinSilver on Windows are:
...
Note that for WSL the BitcoinSilver source path MUST be somewhere in the default mount file system, for
example /usr/src/bitcoinsilver, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
way. This will install to `c:\workspace\bitcoinsilver`, for example:

    make install DESTDIR=/mnt/c/workspace/bitcoinsilver
