# PSBT Howto for BitcoinSilver

Since BitcoinSilver 0.17, an RPC interface exists for Partially Signed BitcoinSilver
Transactions (PSBTs, as specified in
[BIP 174](https://github.com/bitcoinsilver/bips/blob/master/bip-0174.mediawiki)).
...
PSBT is an interchange format for BitcoinSilver transactions that are not fully signed
...
Overall, the construction of a fully signed BitcoinSilver transaction goes through the
following steps:
...
- An **Extractor** produces a valid BitcoinSilver transaction (in network format)
...
#### Multisig with multiple BitcoinSilver instances
...
BitcoinSilver. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `bitcoinsilver-cli` is used.
