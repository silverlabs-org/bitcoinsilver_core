# PSBT Howto for Bitcoin_Silver Core

Since Bitcoin_Silver Core 0.17, an RPC interface exists for Partially Signed Bitcoin_Silver
Transactions (PSBTs, as specified in
[BIP 174](https://github.com/bitcoin_silver/bips/blob/master/bip-0174.mediawiki)).
...
PSBT is an interchange format for Bitcoin_Silver transactions that are not fully signed
...
#### Multisig with multiple Bitcoin_Silver Core instances

Alice, Bob, and Carol want to create a 2-of-3 multisig address. They're all using
Bitcoin_Silver Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `bitcoin_silver-cli` is used.
...
Later, when *V* BTCS has been received on *Amulti*, and Bob and Carol want to
