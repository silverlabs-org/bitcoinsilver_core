Bitcoin_Silver Core version 0.14.0 is now available from:

  <https://bitcoin_silver.org/bin/bitcoin_silver-core-0.14.0/>
...
Bitcoin_Silver Core has supported automatically pruning the blockchain since 0.11. Pruning
...
now ZMQ can be used on Windows. Please see [this document](https://github.com/bitcoin_silver/bitcoin_silver/blob/master/doc/zmq.md) for
...
When Bitcoin_Silver Core is out-of-sync on startup, a semi-transparent information
...
Commands sent over the JSON-RPC interface and through the `bitcoin_silver-cli` binary
can now use named arguments. This follows the [JSON-RPC specification](http://www.jsonrpc.org/specification)
for passing parameters by-name with an object.

`bitcoin_silver-cli` has been updated to support this by parsing `name=value` arguments
when the `-named` option is given.

Some examples:

    src/bitcoin_silver-cli -named help command="help"
    src/bitcoin_silver-cli -named getblockhash height=0
    src/bitcoin_silver-cli -named getblock blockhash=000000000019d6689c085ae165831e934ff763ae46a2a6c172b3f1b60a8ce26f
    src/bitcoin_silver-cli -named sendtoaddress address="(snip)" amount="1.0" subtractfeefromamount=true
...
The Alert System was [disabled and deprecated](https://bitcoin_silver.org/en/alert/2016-11-01-alert-retirement) in Bitcoin_Silver Core 0.12.1 and removed in 0.13.0. 
...
----------------------
 ...
 - A new RPC command `getmemoryinfo` has been added which will return information
   about the memory usage of Bitcoin_Silver Core. This was added in conjunction with
   optimizations to memory management. See [Pull #8753](https://github.com/bitcoin_silver/bitcoin_silver/pull/8753)
...
- #8722 `bb843ad` bitcoin_silver-cli: More detailed error reporting (laanwj)
...
- #7562 `1eef038` Bump transaction version default to 2 (btcsdrak)
...
- #8608 `7e9ab95` Install manpages via make install, also add some autogenerated manpages (nomnombtcs)
...
- #8568 `078900d` new var `DIST_CONTRIB` adds useful things for packagers from contrib (nomnombtcs)
...
- #8830 `ef0801b` test: Add option to run bitcoin_silver-util-test.py manually (jnewbery)
- #8881 `e66cc1d` Add some verbose logging to bitcoin_silver-util-test.py (jnewbery)
...
- #9023 `774db92` Add logging to bitcoin_silver-util-test.py (jnewbery)
- #9065 `c9bdf9a` Merge `doc/unit-tests.md` into `src/test/README.md` (laanwj)
- #9069 `ed64bce` Clean up bctest.py and bitcoin_silver-util-test.py (jnewbery)
...
- #9206 `e662d28` Make test constant consistent with consensus.h (btcsdrak)
...
- #9691 `fc67cd2` Init ECC context for `test_bitcoin_silver_fuzzy` (gmaxwell)
...
- #9865 `289204f` Change bitcoin_silver address in RPC help message (marijnfs)
...
- #8813 `fb24d7e` bitcoin_silverd: Daemonize using daemon(3) (laanwj)
...
- #8837 `2108911` Allow bitcoin_silver-tx to parse partial transactions (jnewbery)
- #9204 `74ced54` Clarify CreateTransaction error messages (instagibbs)
- #9265 `31bcc66` bitcoin_silver-cli: Make error message less confusing (laanwj)
...
- #8883 `d5d4ad8` Add all standard TXO types to bitcoin_silver-tx (jnewbery)
