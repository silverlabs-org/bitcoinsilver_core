Bitcoin_Silver Core version 0.11.0 is now available from:

  <https://bitcoin_silver.org/bin/bitcoin_silver-core-0.11.0/>
...
defaults to 0.00001. This will cause transactions with fewer BTCS/kB fee to be
...
For example, add the following to `bitcoin_silver.conf`:
...
related to the blockchain in the bitcoin_silver system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows Bitcoin_Silver Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that Bitcoin_Silver Core will 
...
For an example script see [bitcoin_silver-submittx](https://github.com/laanwj/bitcoin_silver-submittx).
...
- #6269 `95aca44` gitian: Use the new bitcoin_silver-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only Bitcoin_SilverAmountField
- #5753 `0f44672` Add bitcoin_silver logo to about screen
...
- #5809 `46bfbe7` Add bitcoin_silver-cli man page
...
- #5849 `41259ca` contrib/init/bitcoin_silverd.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename Bitcoin_Silver-Qt.app to Bitcoin_Silver-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for bitcoin_silver-tx)
