Bitcoin_Silver Core version 0.19.0.1 is now available from:

  <https://bitcoin_silvercore.org/bin/bitcoin_silver-core-0.19.0.1/>
...
Bitcoin_Silver Core is supported and extensively tested on operating systems using
the Linux kernel, macOS 10.10+, and Windows 7 and newer. It is not recommended
to use Bitcoin_Silver Core on unsupported systems.
...
to open the Bitcoin_Silver Core .dmg. This is due to new signing requirements
imposed by Apple, which the Bitcoin_Silver Core project does not yet adhere too.
...
- [Reduce memory](https://github.com/bitcoin_silver/bitcoin_silver/blob/master/doc/reduce-memory.md)
  suggests configuration tweaks for running Bitcoin_Silver Core on systems with
...
- `-blockfilterindex` enables the creation of BIP158 block filters for
  ...
  currently use about 4 GiB of space.  Note: this version of Bitcoin_Silver
...
- `whitebind` and `whitelist` now accept a list of permissions to
  ...
  releases.  See the `bitcoin_silverd -help` output for these two options for
...
- In 0.18.0, a `./configure` flag was introduced to allow disabling BIP70 support in the GUI (support was enabled by default). In 0.19.0, this flag is now __disabled__ by default. If you want to compile Bitcoin_Silver Core with BIP70 support in the GUI, you can pass `--enable-bip70` to `./configure`. (#15584)
...
- `generate` is now removed after being deprecated in Bitcoin_Silver Core 0.18.
...
- BIP 61 reject messages were deprecated in v0.18. They are now disabled
  ...
  future version of Bitcoin_Silver Core. (#14054)

- To eliminate well-known denial-of-service vectors in Bitcoin_Silver Core,
  ...
  This prevents Bitcoin_Silver Core from sending the BIP111 NODE_BLOOM service
  ...
  of Bitcoin_Silver Core and nodes that have manually enabled BIP37 support,
...
- By default, Bitcoin_Silver Core will now make two additional outbound connections that are exclusively used for block-relay.  No transactions or addr messages will be processed on these connections. These connections are designed to add little additional memory or bandwidth resource requirements but should make some partitioning attacks more difficult to carry out. (#15759)

Miscellaneous CLI Changes
-------------------------

- The `testnet` field in `bitcoin_silver-cli -getinfo` has been renamed to
...
- `getblockchaininfo` no longer returns a `bip9_softforks` object.
  ...
  an additional `type` field describes how Bitcoin_Silver Core determines
...
- When fetching a transaction announced by multiple peers, previous versions of
  Bitcoin_Silver Core would sequentially attempt to download the transaction from each
...
- If a Tor hidden service is being used, Bitcoin_Silver Core will be bound to
...
- Transactions with outputs paying v1 to v16 witness versions (future
  ...
  and services can accept any valid bech32 Bitcoin_Silver address without
...
- When creating a transaction with a fee above `-maxtxfee` (default 0.1
  BTCS), the RPC commands `walletcreatefundedpsbt` and
  `fundrawtransaction` will now fail instead of rounding down the fee.
  Be aware that the `feeRate` argument is specified in BTCS per 1,000
...
- A new wallet flag `avoid_reuse` has been added (default off). When
  ...
  partial spends" (added in Bitcoin_Silver Core v0.17.0), this can eliminate a
...
- #15649 Add ChaCha20Poly1305@Bitcoin_Silver AEAD (jonasschnelli)
...
- #15639 bitcoin_silver-wallet tool: Drop libbitcoin_server.a dependency (ryanofsky)
...
- #16521 Use the default maxfeerate value as BTCS/kB (Remagpie)
...
- #16366 init: Use InitError for all errors in bitcoin_silverd/qt (MarcoFalke)
...
- #15239 scripts and tools: Move non-linux build source tarballs to "bitcoin_silver-binaries/version" directory (hebasto)
...
- #16258 [MSVC]: Create the config.ini as part of bitcoin_silverd build (NicolasDorier)
...
- #16441 remove qt libjpeg check from bitcoin_silver_qt.m4 (fanquake)
...
- #16750 msbuild: adds bench_bitcoin_silver to auto generated project files (sipsorcery)
...
- #15773 Add Bitcoin_SilverTestFramework::sync_* methods (MarcoFalke)
...
- #15352 Reduce noise level in test_bitcoin_silver output (practicalswift)
...
- #15896 feature_filelock, interface_bitcoin_silver_cli: Use PACKAGE_NAME in messages rather than hardcoding Bitcoin_Silver Core (luke-jr)
...
- #16277 Suppress output in test_bitcoin_silver for expected errors (gertjaap)
...
- #16929 follow-up to rpc: default maxfeerate value as BTCS/kB (jonatack)
...
- #16489 log: harmonize bitcoin_silverd logging (jonatack)
...
- #17449 fix uninitialized variable nMinerConfirmationWindow (bitcoin_silverVBR)

### Documentation
- #15514 Update Transifex links (fanquake)
- #15513 add "sections" info to example bitcoin_silver.conf (fanquake)
...
- #16080 build/doc: update bitcoin_silver_config.h packages, release process (jonatack)
...
- #16448 add note on precedence of options in bitcoin_silver.conf (fanquake)
...
- #16621 add default bitcoin_silver.conf locations (GChuf)
...
- #16863 Add a missing closing parenthesis in the bitcoin_silver-wallet's help (darosior)
- #16757 CChainState return values (MarcoFalke)
- #16847 add comments clarifying how local services are advertised (jamesob)
- #16812 Fix whitespace errs in .md files, bitcoin_silver.conf, and Info.plist.in (ch4ot1c)
...
- #16912 Remove Doxygen intro from src/bitcoin_silverd.cpp (ch4ot1c)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/bitcoin_silver/bitcoin_silver/).
