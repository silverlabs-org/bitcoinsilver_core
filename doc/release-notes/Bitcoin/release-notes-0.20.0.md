...
Bitcoin_Silver Core version 0.20.0 is now available from:

  <https://bitcoin_silvercore.org/bin/bitcoin_silver-core-0.20.0/>
...
- Instead of running `make` simply, you should instead run
  `BITCOIN_SILVER_GENBUILD_NO_GIT=1 make`.
...
#### Removal of BIP61 reject network messages from Bitcoin_Silver Core

The `-enablebip61` command line option to enable BIP61 has been removed.
(#17004)

This feature has been disabled by default since Bitcoin_Silver Core version 0.18.0.
...
- Testing or debugging of implementations of the Bitcoin_Silver P2P network protocol
  should be done by inspecting the log messages that are produced by a recent
  version of Bitcoin_Silver Core. Bitcoin_Silver Core logs debug messages
...
- OpenSSL is no longer used by Bitcoin_Silver Core.  (#17265)

- BIP70 support has been fully removed from Bitcoin_Silver Core. The
...
- A new `-asmap` configuration option has been added to diversify the
  ...
  single ASN.  See [issue #16599](https://github.com/bitcoin_silver/bitcoin_silver/issues/16599),
  [PR #16702](https://github.com/bitcoin_silver/bitcoin_silver/pull/16702), and the
  `bitcoin_silverd help` for more information.  This option is experimental and
...
- All custom settings configured when Bitcoin_Silver Core starts are now
...
- The "Start Bitcoin_Silver Core on system login" option has been removed on macOS.
  (#17567)
...
- A "known bug" [announced](https://bitcoin_silvercore.org/en/releases/0.18.0/#wallet-gui)
  in the release notes of version 0.18 has been fixed.  The issue
  affected anyone who simultaneously used multiple Bitcoin_Silver Core wallets
...
- For watch-only wallets, creating a new transaction in the Send screen
  ...
  automatically copy a Partially-Signed Bitcoin_Silver Transaction (PSBT) to
  the system clipboard.  This can then be pasted into an external
  program such as [HWI](https://github.com/bitcoin_silver-core/HWI) for
  signing.  Future versions of Bitcoin_Silver Core should support a GUI option
...
- Bitcoin_Silver Core's automatically-generated source code documentation is
  now available at https://doxygen.bitcoin_silvercore.org.  (#17596)
...
- The `bitcoin_silver-cli` utility used with the `-getinfo` parameter now
...
- The `bitcoin_silver-cli` utility now accepts a `-stdinwalletpassphrase`
...
- #17068 Always generate `bitcoin_silverstrings.cpp` on `make translate` (D4nte)
...
- #17741 Included `test_bitcoin_silver-qt` in msvc build (sipsorcery)
...
- #18397 Fix libevent linking for `bench_bitcoin_silver` binary (hebasto)
...
- #18702 Fix ASLR for bitcoin_silver-cli on Windows (fanquake)
...
- #17206 Add testcase to simulate bitcoin_silver schema in leveldb (adamjonas)
...
- #17330 Add `shrinkdebugfile=0` to regtest bitcoin_silver.conf (sdaftuar)
...
- #18022 Fix appveyor `test_bitcoin_silver` build of `*.raw` (MarcoFalke)
...
- #18166 ci: Run fuzz testing test cases (bitcoin_silver-core/qa-assets) under valgrind to catch memory errors (practicalswift)
...
- #18430 ci: Only clone bitcoin_silver-core/qa-assets when fuzzing (MarcoFalke)
...
- #16983 Add detailed info about Bitcoin_Silver Core files (hebasto)
...
- #17119 Fix broken bitcoin_silver-cli examples (andrewtoth)
...
- #17503 Remove bitness from bitcoin_silver-qt help message and manpage (laanwj)
- #17539 Update and improve Developer Notes (hebasto)
- #17561 Changed MiniUPnPc link to https in dependencies.md (sandakersmann)
- #17596 Change doxygen URL to doxygen.bitcoin_silvercore.org (laanwj)
...
- #17648 Rename wallet-tool references to bitcoin_silver-wallet (hel-o)
...
- #18341 Replace remaining literal BTCS with `CURRENCY_UNIT` (domob1812)
...
- #16115 On bitcoin_silverd startup, write config args to debug.log (LarryRuane)
...
- #17650 util: Remove unwanted fields from bitcoin_silver-cli -getinfo (malevolent)
