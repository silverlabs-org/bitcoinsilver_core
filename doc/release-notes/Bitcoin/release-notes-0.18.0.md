Bitcoin_Silver Core version 0.18.0 is now available from:

  <https://bitcoin_silvercore.org/bin/bitcoin_silver-core-0.18.0/>
...
`/Applications/Bitcoin_Silver-Qt` (on Mac) or `bitcoin_silverd`/`bitcoin_silver-qt` (on
Linux).
...
recommended to use Bitcoin_Silver Core on unsupported systems.

Bitcoin_Silver Core should also work on most other Unix-like systems but is not
as frequently tested on them.

From 0.17.0 onwards, macOS <10.10 is no longer supported. 0.17.0 is
built using Qt 5.9.x, which doesn't support versions of macOS older than
10.10. Additionally, Bitcoin_Silver Core does not yet change appearance when
...
`contrib/init/bitcoin_silverd.service`, it has been changed to use
`/var/lib/bitcoin_silverd` as the data directory instead of
`~bitcoin_silver/.bitcoin_silver`. When switching over to the new configuration file,
please make sure that the filesystem on which `/var/lib/bitcoin_silverd` will
exist has enough space (check using `df -h /var/lib/bitcoin_silverd`), and
...
- Four new options are available for configuring the maximum number of
  ...
  documentation](https://github.com/bitcoin_silver/bitcoin_silver/blob/master/doc/zmq.md#usage)
  for details.
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](https://bitcoin_silvercore.org/en/contact/) the Bitcoin_Silver Core
...
The systemd init file (`contrib/init/bitcoin_silverd.service`) has been changed
to use `/var/lib/bitcoin_silverd` as the data directory instead of
`~bitcoin_silver/.bitcoin_silver`. This change makes Bitcoin_Silver Core more consistent with
...
`datadir` option in `/etc/bitcoin_silver/bitcoin_silver.conf` will have no effect.
This is because the command line arguments specified in the init files
take precedence over the options specified in
`/etc/bitcoin_silver/bitcoin_silver.conf`.


Documentation
-------------

- A new short [document](https://github.com/bitcoin_silver/bitcoin_silver/blob/master/doc/JSON-RPC-interface.md)
  about the JSON-RPC interface describes cases where the results of an
  RPC might contain inconsistencies between data sourced from different
  subsystems, such as wallet state and mempool state.  A note is added
  to the [REST interface documentation](https://github.com/bitcoin_silver/bitcoin_silver/blob/master/doc/REST-interface.md)
  indicating that the same rules apply.

- Further information is added to the [JSON-RPC
  documentation](https://github.com/bitcoin_silver/bitcoin_silver/blob/master/doc/JSON-RPC-interface.md)
  about how to secure this interface.

- A new [document](https://github.com/bitcoin_silver/bitcoin_silver/blob/master/doc/bitcoin_silver-conf.md)
  about the `bitcoin_silver.conf` file describes how to use it to configure
  Bitcoin_Silver Core.

- A new document introduces Bitcoin_Silver Core's BIP174 [Partially-Signed
  Bitcoin_Silver Transactions
  (PSBT)](https://github.com/bitcoin_silver/bitcoin_silver/blob/master/doc/psbt.md)
...
- The [output script
  descriptor](https://github.com/bitcoin_silver/bitcoin_silver/blob/master/doc/descriptors.md)
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent Bitcoin_Silver-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  Bitcoin_Silver Core to libssl vulnerabilities in the past, builders who don't
...
- The minimum required version of Qt (when building the GUI) has been
  ...
  system](https://github.com/bitcoin_silver/bitcoin_silver/blob/master/depends/README.md)
  provides 5.9.7)
...
- `deriveaddresses` returns one or more addresses corresponding to an
  [output descriptor](https://github.com/bitcoin_silver/bitcoin_silver/blob/master/doc/descriptors.md).
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when Bitcoin_Silver Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of Bitcoin_Silver Core, subsequently importing
...
- `importmulti` now supports importing of addresses from descriptors. A
  ...
  [here](https://github.com/bitcoin_silver/bitcoin_silver/blob/master/doc/descriptors.md).
...
- The 'account' API is removed after being deprecated in v0.17.  The
  ...
  v0.17](https://github.com/bitcoin_silver/bitcoin_silver/blob/master/doc/release-notes/release-notes-0.17.0.md#label-and-account-apis-for-wallet)
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart bitcoin_silverd with the `-deprecatedrpc=generate`
...
- A new `bitcoin_silver-wallet` tool is now distributed alongside Bitcoin_Silver
...
This section describes planned changes to Bitcoin_Silver Core that may affect
other Bitcoin_Silver software and services.

- Since version 0.16.0, Bitcoin_Silver Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with Bitcoin_Silver Core 0.20 (expected about
  a year after 0.18), Bitcoin_Silver Core will default to native segwit
  ...
  to bech32 addresses, and if the Bitcoin_Silver Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in Bitcoin_Silver Core 0.19 (approximately November 2019).
  ...
  set the `addresstype=bech32` configuration option in any Bitcoin_Silver Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, Bitcoin_Silver Core
...
- A sub-project of Bitcoin_Silver Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with Bitcoin_Silver Core.  See their [project
  page](https://github.com/bitcoin_silver-core/HWI#readme) for details.

Security
--------

- This release changes the Random Number Generator (RNG) used from
  OpenSSL to Bitcoin_Silver Core's own implementation, although entropy
  gathered by Bitcoin_Silver Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves Bitcoin_Silver Core a
...
- On macOS, Bitcoin_Silver Core now opts out of application CPU throttling
...
- #12490 Remove deprecated wallet rpc features from `bitcoin_silver_server` (jnewbery)
...
- #13926 [Tools] bitcoin_silver-wallet - a tool for creating and managing wallets offline (jnewbery)
...
- #14162 Also log and print messages or questions like bitcoin_silverd (MarcoFalke)
...
- #14521 qt, docs: Fix `bitcoin_silver-qt -version` output formatting (hebasto)
...
- #11625 Add Bitcoin_SilverApplication & RPCConsole tests (ryanofsky)
...
- #15154 Configure: bitcoin_silver-tx doesn't need libevent, so don't pull it in (luke-jr)
...
- #15388 Makefile.am: add rule for src/bitcoin_silver-wallet (Sjors)
...
- #11526 Visual Studio build configuration for Bitcoin_Silver Core (sipsorcery)
...
- #13953 Fix deprecation in bitcoin_silver-util-test.py (isghe)
...
- #12775 Integration of property based testing into Bitcoin_Silver Core (Christewart)
...
- #12246 Bugfix: Only run bitcoin_silver-tx tests when bitcoin_silver-tx is enabled (luke-jr)
...
- #14092 Dry run `bench_bitcoin_silver` as part `make check` to allow for quick identification of assertion/sanitizer failures in benchmarking code (practicalswift)
...
- #14985 Remove `thread_local` from `test_bitcoin_silver` (MarcoFalke)
...
- #14186 bitcoin_silver-cli: don't translate command line options (HashUnlimited)
- #14057 logging: Only log `using config file path_to_bitcoin_silver.conf` message on startup if conf file exists (leishman)
...
- #12255 Update bitcoin_silver.service to conform to init.md (dongcarl)
...
- #14497 Add doc/bitcoin_silver-conf.md (hebasto)
...
- #15354 Add missing `bitcoin_silver-wallet` tool manpages (MarcoFalke)
...
- #15765 correct bitcoin_silverconsensus_version in shared-libraries.md (fanquake)
- #15792 describe onlynet option in doc/tor.md (jonatack)
- #15802 mention creating application support bitcoin_silver folder on OSX (JimmyMow)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/projects/p/bitcoin_silver/).
