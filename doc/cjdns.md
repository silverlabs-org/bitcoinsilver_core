# CJDNS support in BitcoinSilver

It is possible to run BitcoinSilver over CJDNS, an encrypted IPv6 network that
...
done outside of BitcoinSilver, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the BitcoinSilver network and individual nodes.
...
before it will work with your BitcoinSilver node. This is described in steps
...
## Run BitcoinSilver with CJDNS

Once you are connected to the CJDNS network, the following BitcoinSilver
...
When enabled, this option tells BitcoinSilver that it is running in an
...
IPv6 local network. This helps BitcoinSilver perform better address management:
...
CJDNS support was added to BitcoinSilver in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `bitcoinsilver-cli -addrinfo` to see the
...
There are several ways to see your CJDNS address in BitcoinSilver:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `bitcoinsilver-cli -netinfo 4`
