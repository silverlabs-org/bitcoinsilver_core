...
This guide describes how to build bitcoinsilverd, command-line utilities, and GUI on OpenBSD.
...
Clone the BitcoinSilver repository to a directory. All build scripts and commands will run from this directory.
``` bash
...
It is not necessary to build wallet functionality to run either `bitcoinsilverd` or `bitcoinsilver-qt`.
...
...
to: /path/to/bitcoinsilver/depends/x86_64-unknown-openbsd
```

Then set `BDB_PREFIX`:

```bash
export BDB_PREFIX="/path/to/bitcoinsilver/depends/x86_64-unknown-openbsd"
```

#### GUI Dependencies
###### Qt5

BitcoinSilver includes a GUI built with the cross-platform Qt Framework. To compile the GUI, Qt 5 is required.
...
(see issue [#6658](https://github.com/MrVistos/bitcoinsilver/issues/6658)).
